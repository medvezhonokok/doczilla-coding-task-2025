...
\begin{document}
    ...
    \end{algorithmic}
    \vspace{15pt}
    Таким образом, у нас в \textit{root} будет лежать последний шаг, который привел к решению. Тогда восстановить ответ можно просто перебирая его родителей \textit{(root.parent)} пока он есть.
    \vspace{15pt}
    \begin{algorithmic}[1]
        \Procedure{getSolution}{$tail$}
            \State $history \gets \text{[]}$

            \While{$tail \neq \text{null}$}
                \State $history \gets history \cup \{current\}$
                \State $tail \gets tail.parent$
            \EndWhile

            \State \Return $history.\text{reversed()}$ // т.к. идем с последнего хода до первого
        \EndProcedure
    \end{algorithmic}
    \vspace{15pt}
    \vspace{15pt}
